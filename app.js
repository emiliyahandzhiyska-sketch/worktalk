...
const ENCOURAGEMENTS = [
  ...
  { en: "You will forget words. That isn't failure, it's how memory works.", bg: "Ще забравяш думи. Това не е провал, така работи паметта." },
  { en: "The meeting won't wait until you're fluent. Go anyway.", bg: "Срещата няма да чака да проговориш свободно. Влез така или иначе." },
  { en: "Nobody judges your English as harshly as you do.", bg: "Никой не съди английския ти толкова строго, колкото ти самият." },
  { en: "Three words in the right order beat a perfect sentence you never said.", bg: "Три думи в правилния ред бият перфектно изречение, което не си казал." },
  { en: "Missing a day is normal. Missing a week is a decision.", bg: "Един пропуснат ден е нормално. Пропусната седмица вече е решение." },
  { en: "You learned your first language by using it badly for years. Same here.", bg: "Първия си език си научил, като си го говорил зле с години. Тук е същото." },
  { en: "The colleague you envy also rehearses in the shower.", bg: "Колегата, на когото завиждаш, също репетира под душа." },
  { en: "Fluent doesn't mean fast. It means not stopping.", bg: "Свободно не значи бързо. Значи да не спираш." },
  { en: "Write the email. Send it. Improve it in the next one.", bg: "Напиши имейла. Изпрати го. Подобри следващия." },
  { en: "Learning at 40 goes faster than at 14. You know what you need.", bg: "На 40 се учи по-бързо, отколкото на 14. Вече знаеш какво ти трябва." },
  { en: "Ten minutes on a bad day counts double.", bg: "Десет минути в лош ден се броят двойно." },
  { en: "You don't need more words. You need the same words more often.", bg: "Не ти трябват повече думи. Трябват ти същите думи по-често." },
  { en: "Silence in a meeting costs more than a wrong preposition.", bg: "Мълчанието на среща струва повече от сбъркан предлог." },
  { en: "Progress feels like nothing until you hear yourself on a call.", bg: "Напредъкът изглежда като нищо, докато не се чуеш на разговор." },
  { en: "Start with the phrase you needed last week.", bg: "Започни с фразата, която ти трябваше миналата седмица." },
  { en: "A plateau means the easy part is behind you.", bg: "Платото значи, че лесната част е зад гърба ти." },
  { en: "You're allowed to sound simple. Simple is clear.", bg: "Имаш право да звучиш просто. Простото е ясно." },
  { en: "The first sentence is the hard one. The rest follow it.", bg: "Първото изречение е трудното. Останалите вървят след него." },
  { en: "Ask them to repeat. Everyone does it, in every language.", bg: "Помоли ги да повторят. Всички го правят, на всеки език." },
  { en: "Two weeks in, it stops being effort and starts being routine.", bg: "След две седмици спира да е усилие и става рутина." },
  { en: "You don't have to enjoy it today. You have to open it today.", bg: "Не е нужно да ти е приятно днес. Нужно е да го отвориш днес." },
  { en: "Reading is easier than speaking. Start where it's easy.", bg: "Четенето е по-лесно от говоренето. Започни от лесното." },
  { en: "One good phrase per meeting is a plan.", bg: "Една добра фраза на среща вече е план." },
  { en: "Your notes aren't learning. Using them is.", bg: "Бележките ти не са учене. Ползването им е." },
  { en: "Confidence comes from repetition, not from talent.", bg: "Увереността идва от повторение, не от талант." },
  { en: "The people who improved fastest sounded worst at the start.", bg: "Тези, които напреднаха най-бързо, звучаха най-зле в началото." },
  { en: "Say it badly today so you can say it well in March.", bg: "Кажи го зле днес, за да го кажеш добре през март." },
  { en: "Ten cards is a session. Two cards is also a session.", bg: "Десет карти са сесия. Две карти също са сесия." },
  { en: "Being understood is the whole job. The rest is decoration.", bg: "Да те разберат е цялата работа. Останалото е украса." },
  { en: "You practise now so the words are there when you're nervous.", bg: "Упражняваш се сега, за да са налице думите, когато си притеснен." },
  { en: "Tired brains still learn. Just give them fewer cards.", bg: "Уморените мозъци пак учат. Просто им дай по-малко карти." },
  { en: "The word you looked up twice is the word worth learning.", bg: "Думата, която си проверявал два пъти, е думата за учене." },
  { en: "Nobody starts a language feeling clever. That arrives around month six.", bg: "Никой не започва език и се чувства умен. Това идва някъде към шестия месец." },
  { en: "Your job already uses about 200 English words. Learn those first.", bg: "Работата ти вече ползва към 200 английски думи. Научи първо тях." },
  { en: "The best time to practise is the time you'll actually do it.", bg: "Най-доброто време за учене е това, в което наистина ще го правиш." },
  { en: "You're not slow. You're busy, which is a different thing.", bg: "Не си бавен. Зает си, а това е друго." },
        ...
        <div class="space-y-2.5">
          ${points.map(g => `
            <button onclick="openGrammarTopic('${g.id}')"
              ...
              ${(() => {
                const rounds = grammarRounds(g);
                const clean = grammarRoundsClean(g);
                if (!rounds.some(r => best[r.key] !== undefined)) return '';
                return `<span class="text-xs font-bold whitespace-nowrap ${clean === rounds.length
                  ? 'text-emerald-500' : 'text-brand-500'}">${clean === rounds.length ? '🏆 ' : ''}${clean}/${rounds.length} rounds</span>`;
              })()}
            </button>`).join('')}
        </div>`).join('')}
...
function renderGrammarExplanation() {
  ...
  document.getElementById('grammarBox').innerHTML = `
    <div class="fade-in">
      ...
      <p class="text-[11px] font-bold uppercase tracking-widest text-slate-400 mt-5 mb-2">Practice · ${grammarRoundTotal(point)} questions</p>
      <div class="space-y-2.5">
        ${grammarRounds(point).map((rd, i) => {
          const best = grammarBest()[rd.key];
          const done = best === rd.count;
          return `
          <button onclick="${rd.start}()"
            class="w-full text-left px-4 py-3.5 rounded-xl border transition-colors flex items-center gap-3 ${i === 0
              ? 'border-brand-500 bg-brand-500 text-white hover:bg-brand-600'
              : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:border-brand-500'}">
            <span class="text-xl">${rd.icon}</span>
            <span class="flex-1">
              <span class="block font-bold text-sm">${rd.title}</span>
              <span class="block text-xs ${i === 0 ? 'text-white/75' : 'text-slate-500 dark:text-slate-400'}">${rd.desc}</span>
            </span>
            <span class="text-xs font-bold whitespace-nowrap ${i === 0 ? 'text-white/90' : 'text-brand-500'}">${
              best === undefined ? rd.count : `${done ? '✓ ' : ''}${best}/${rd.count}`}</span>
          </button>`; }).join('')}
      </div>
    </div>`;
}

// The four ways to practise one point. Rounds with nothing behind them
// (a point with no traps) simply don't appear.
function grammarRounds(point) {
  const traps = mistakesForPoint(point).length;
  return [
    { key: point.id, icon: '🧩', title: 'Choose the right form', desc: 'Pick one of two answers', count: point.exercises.length, start: 'startGrammarQuiz' },
    { key: point.id + ':typing', icon: '⌨️', title: 'Type it yourself', desc: 'Same sentences, no options to pick from', count: point.exercises.length, start: 'startGrammarTyping' },
    ...(traps ? [{ key: point.id + ':mistakes', icon: '🚫', title: 'Spot the mistake', desc: `${traps} Bulgarian trap${traps > 1 ? 's' : ''}`, count: traps, start: 'startGrammarMistakes' }] : []),
    { key: point.id + ':translate', icon: '🇧🇬', title: 'Translate from Bulgarian', desc: 'The examples, back to front', count: point.examples.length, start: 'startGrammarTranslate' }
  ];
}

function grammarRoundTotal(point) {
  return grammarRounds(point).reduce((n, r) => n + r.count, 0);
}

// How many of a point's rounds have been finished without a single slip.
function grammarRoundsClean(point) {
  const best = grammarBest();
  return grammarRounds(point).filter(r => best[r.key] === r.count).length;
}
...
// The same five sentences with the options taken away. Choosing between two
// answers is recognition; typing one is production, and it is much harder.
function startGrammarTyping() {
  const point = grammarTopic.point;
  grammarTopic.items = shuffle(point.exercises.map(e => ({
    sentence: e.sentence,
...
// The examples, back to front: the Bulgarian is the question and the English
// is the answer. Nothing new is invented, the sentences are the ones above.
function startGrammarTranslate() {
  const point = grammarTopic.point;
  grammarTopic.items = shuffle(point.examples.map(ex => ({
    prompt: ex.bg,
    answer: ex.en,
    typed: true,
    translate: true
  })));
  grammarTopic.mode = 'translate';
  grammarTopic.index = 0;
  grammarTopic.score = 0;
  grammarTopic.checked = false;
  renderGrammarQuestion();
...
// Typed answers are checked loosely on purpose: punctuation, capitals and
// contractions shouldn't decide whether somebody understood the grammar.
// A translation counts when every content word of the model is in there.
function grammarTypedCorrect(it, raw) {
  const said = normalize(raw).replace(/[’']/g, '');
  const want = normalize(it.answer).replace(/[’']/g, '');
  if (!said) return false;
  if (said === want) return true;
  if (!it.translate) return false;
  const skip = new Set(['a', 'an', 'the', 'to', 'of', 'and', 'is', 'are', 'am', 'be', 'in', 'on', 'at', 'it', 'that', 'this']);
  const words = want.split(' ').filter(w => w.length > 2 && !skip.has(w));
  if (!words.length) return said === want;
  return words.every(w => said.includes(w.replace(/(ing|ed|s)$/, '')));
}

function grammarCheckTyped() {
  if (grammarTopic.checked) return;
  const input = document.getElementById('grammarAnswer');
  if (!input.value.trim()) return;
  grammarTopic.checked = true;
  const it = grammarTopic.items[grammarTopic.index];
  const correct = grammarTypedCorrect(it, input.value);
  if (correct) grammarTopic.score++;
  input.disabled = true;
  input.className += correct
    ? ' !border-emerald-500 !bg-emerald-50 dark:!bg-emerald-900/30'
    : ' !border-rose-500 !bg-rose-50 dark:!bg-rose-900/25';
  showGrammarFeedback(correct, it, `<p class="text-sm mb-1"><span class="text-slate-500 dark:text-slate-400">Answer:</span> <b>${it.answer}</b></p>`);
}
...
const GRAMMAR_ROUND_ICON = { rule: '', typing: '⌨️ ', mistakes: '🚫 ', translate: '🇧🇬 ' };
...
function renderGrammarQuestion() {
  ...
  const head = `
    <div class="flex items-center justify-between mb-4">
      <span class="text-xs font-bold text-brand-500">${GRAMMAR_ROUND_ICON[mode] || ''}${point.title}</span>
      <span class="text-xs text-slate-500 dark:text-slate-400">${index + 1} / ${items.length} · Score: ${grammarTopic.score}</span>
    </div>`;

  const body = it.typed ? `
      ${it.translate ? '<p class="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-2">Say this in English</p>' : ''}
      <p class="text-lg leading-relaxed mb-5">${it.translate ? it.prompt : it.sentence}</p>
      <input id="grammarAnswer" type="text" autocomplete="off" autocapitalize="off" spellcheck="false"
        placeholder="${it.translate ? 'Type the English sentence' : 'Type the missing words'}"
        onkeydown="if(event.key==='Enter')grammarCheckTyped()"
        class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:border-brand-500 mb-3">
      <button onclick="grammarCheckTyped()" class="w-full py-3 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-sm transition-colors">Check</button>`
    : `
  ...
  document.getElementById('grammarBox').innerHTML =
    `<div class="fade-in">${head}${body}<div id="grammarFeedback" class="mt-3"></div></div>`;
  if (it.typed) document.getElementById('grammarAnswer').focus();
}
...
function grammarAnswer(i) {
  ...
  showGrammarFeedback(correct, it);
}
...
function showGrammarFeedback(correct, it, extra = '') {
  const why = it.why ? `
...
const GRAMMAR_ROUND_KEY = { rule: '', typing: ':typing', mistakes: ':mistakes', translate: ':translate' };

function renderGrammarEnd() {
  const { point, score, items, mode } = grammarTopic;
  const total = items.length;
  // Every round keeps its own best, so a first attempt at the typing
  // doesn't overwrite a clean score on the multiple choice.
  const bestKey = point.id + GRAMMAR_ROUND_KEY[mode];
  ...
  const clean = grammarRoundsClean(point);
  const mastered = clean === rounds.length;

  const msg = mastered ? 'Every round on this point is clean. 🏆'
    : score === total ? 'Perfect round. One more to go on this point.'
    : score >= total * 0.7 ? 'Strong result. One more pass and it will stick.'
    : 'Good practice. Reread the rule above and try again.';

  // Offer the first round that hasn't been finished cleanly yet, so the
  // point walks a learner through all of it without them choosing each time.
  const best = grammarBest();
  const nextRound = rounds.find(r => best[r.key] !== r.count);
  const nextButton = nextRound
    ? `<button onclick="${nextRound.start}()" class="px-6 py-3 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-sm transition-colors">${nextRound.icon} ${nextRound.title}</button>`
    : `<button onclick="startGrammarQuiz()" class="px-6 py-3 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-sm transition-colors">Go round again</button>`;

  document.getElementById('grammarBox').innerHTML = `
    <div class="text-center py-6 fade-in">
      <p class="text-4xl mb-3">${mastered ? '🏆' : score >= total * 0.7 ? '🎉' : '💪'}</p>
      <h2 class="text-xl font-extrabold mb-2">${point.title}: ${score} / ${total}</h2>
      ${isRecord ? '<p class="text-sm font-semibold text-emerald-500 mb-2">New personal best!</p>' : ''}
      <p class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">${clean} of ${rounds.length} rounds clean</p>
      <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">${msg}</p>
